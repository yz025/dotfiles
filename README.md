# dotfiles
## Table
* Windows 11 (`w11_desktop`)
    * [nvim](https://github.com/neovim/neovim) 0.12.2 with [mini.nvim](https://github.com/nvim-mini/mini.nvim), [minimax](https://github.com/nvim-mini/MiniMax)
    * pwsh 7.6.0
    * [alacritty](https://github.com/alacritty/alacritty)
    * [wezterm](https://github.com/wezterm/wezterm) (archived)
    * [winfetch](https://github.com/lptstr/winfetch)
    * [yazi](https://github.com/sxyazi/yazi)
    * [starship](https://github.com/starship/starship)
    * [.clang-format](https://clang.llvm.org/docs/ClangFormat.html)
    * [stylua](https://github.com/JohnnyMorganz/StyLua)
* Fedora 43 (`f43_desktop`, `f43_wsl`(likely outdated))
    * [nvim](https://github.com/neovim/neovim) 0.11.6 with [lazy.nvim](https://github.com/folke/lazy.nvim)
    * bash
    * [alacritty](https://github.com/alacritty/alacritty)
    * [yazi](https://github.com/sxyazi/yazi)
    * [starship](https://github.com/starship/starship)
    * [.clang-format](https://clang.llvm.org/docs/ClangFormat.html)
* Theme
    * [rasmus](https://github.com/kvrohit/rasmus.nvim)
## Notes
`ddf-list.toml` in each OS is for [the tool I made](https://github.com/yz-5555/ddf)
